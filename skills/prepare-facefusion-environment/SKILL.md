...
2. If `facefusion_health_check` shows `needs_install` or `needs_setup`, explain what is missing and ask for confirmation before calling `facefusion_install_or_setup`.
3. If the user asks what is available, call `facefusion_list_capabilities`.
4. If the install is present but missing or repairing models, call `facefusion_download_models`.
5. If the user asks about speed, providers, or tuning, read `../../resources/execution-providers.md` and call `facefusion_benchmark`.
...
- Do not call `facefusion_install_or_setup` without explicit user confirmation.
